...
namespace MyTcpSockets.Extensions
{
    public static class TcpReaderExtensions
    {
        ...
        public static async ValueTask<ushort> ReadUShortAsync(this TcpDataReader reader, CancellationToken token)
        {
            ...
            reader.CommitReadData(data.Length);
            return result;
        }     
        ...
        public static async ValueTask<byte[]> ReadByteArrayAsync(this TcpDataReader reader, CancellationToken token)
        {
            var strLen = await reader.ReadIntAsync(token);
            var result= (await reader.ReadAsyncAsync(strLen, token)).ToArray();
            reader.CommitReadData(strLen);
            return result;
        }
